...
/* ===========================================  Includes  =========================================== */
#include "ckgen_hw.h"
#include "../src/rcm/rcm_hw.h"
#include "../src/spm/spm_hw.h"
...
/* ==========================================  Variables  =========================================== */
static clock_manager_state_t g_clockState;       /* Clock manager state */
...
static void CKGEN_SetSimConfiguration(const sim_clock_config_t *simClockConfig)
{
    ...
    if (simClockConfig->tclkConfig.initialize)
    {
        ...
        for (i = 0U; i < PWM_INSTANCE_MAX; i++)
        {
            CKGEN_SetPWMClock(i, simClockConfig->tclkConfig.source[i]);
        }
    }
...
static void CKGEN_GetDefaultConfiguration(clock_manager_user_config_t *config)
{
    ...
    for (i = 0; i < PWM_INSTANCE_MAX; i++)
    {
        config->simConfig.tclkConfig.source[i]            = HSIDIV1_CLK;
    }
...
static status_t CKGEN_GetPccClockFreq(clock_names_t clockName, uint32_t *frequency)
{
    ...
    if (((uint32_t)clockName >= CKGEN_PERI_CLK_BASE) && (clockName < PWM0_CLK))
    {
        ...
        if ((clockName >= CAN0_CLK) && (clockName < PCT_CLK))
        {
            ...
            source = CKGEN_GetCANClock(instance);
            returnCode = CKGEN_DRV_GetFreq(source, &freq);
            divider = CKGEN_GetCANClockDivider(instance);

            freq = freq / ((uint32_t)divider + 1U);
        }
        else /* Set other module clock source in clock mux0~2 */
        {
            source = (clock_names_t)((CKGEN_GetPeripheralClock(clockName) & 0x3) + (uint8_t)HSEDIV2_CLK);
            returnCode = CKGEN_DRV_GetFreq(source, &freq);
        }
    }
    else if ((clockName >= PWM0_CLK) && (clockName < PCC_END_OF_CLOCKS)) /* Set PWM clock source */
    {
        ...
        if (pwmExtClockEn) /* PWM external clock source */
        {
...
static status_t CKGEN_ConfigureSPLL(bool enable, const ckgen_spll_config_t *spllConfig)
{
    ...
    uint32_t srcFreq = 0U, timeout, div2 = 0U;
    ...
    if (enable && (STATUS_SUCCESS == status))
    {
        ...
        /* Setup SPLL dividers. */
        srcFreq /= (spllCfg->posdiv * 2U);
        div2 = (uint32_t)spllCfg->div2;
        /* SPLLDIV2 should not be bigger than BUS clock */
        if (srcFreq > 60000000U)
        {
            /* When the splldiv clock is out of range, reduce the frequency automatically */
            if (CKGEN_ASYNC_CLOCK_DIV_BY_1 == spllCfg->div2)
            {
                div2 = (uint32_t)CKGEN_ASYNC_CLOCK_DIV_BY_2;
            }
        }
        CKGEN_SetSPLLAsyncConfig(CKGEN, (uint32_t)spllCfg->div1, div2);
...
{
    status_t status = STATUS_SUCCESS;
    uint32_t srcFreq = 0U, sysFreq = 0U;
    ...
    const uint32_t sysFreqDiv = ((uint32_t)config->divCore) + 1U;
    const uint32_t busFreqDiv = sysFreqDiv * (((uint32_t)config->divBus) + 1U);

    DEVICE_ASSERT(mode != CKGEN_SYSTEM_CLOCK_MODE_CURRENT);

    srcFreq = CKGEN_GetSrcFreq(config->src);
    sysFreq = srcFreq / sysFreqDiv;
    ...
    case CKGEN_SYSTEM_CLOCK_MODE_RUN:       /* Run mode. */
        ...
        if ((sysFreq > maxSysClksInRUN[(uint32_t)config->src][CORE_CLK_INDEX]) ||              /* Sys(core) clock */
            ((srcFreq / busFreqDiv) > maxSysClksInRUN[(uint32_t)config->src][BUS_CLK_INDEX]))  /* Bus clock */
        {
    ...
    case CKGEN_SYSTEM_CLOCK_MODE_VLPR:      /* Very low power run mode. */
        ...
        if ((sysFreq > maxSysClksInVLPR[0][CORE_CLK_INDEX]) ||                /* Sys(core) clock */
            ((srcFreq / busFreqDiv) > maxSysClksInVLPR[0][BUS_CLK_INDEX]) ||  /* Bus clock */
...
static uint32_t CKGEN_GetSPLLFreq(void)
{
    ...
    if (SPM_GetSPLLStatus()) /* System PLL is valid. */
    {
        ...
        if (freq != 0U) /* If source is valid. */
        {
            ...
            freq /= (prediv + CKGEN_SPLL_PREDIV_BASE);  /* Pre-divider. */
            freq *= fbkdiv;                             /* Fbk-divider. */
            freq /= posdiv;                             /* Pos-divider. */
    ...
    if (moduleClkConfig != NULL)
    {
        ...
        if (((uint32_t)peripheralClock >= CKGEN_PERI_CLK_BASE) && (peripheralClock < PWM0_CLK))
        {
            ...
            if ((peripheralClock >= CAN0_CLK) && (peripheralClock < PCT_CLK))
            {
                ...
                CKGEN_SetCANClock(instance, moduleClkConfig->source);
                CKGEN_SetCANClockDivider(instance, (can_clock_divider_t)(moduleClkConfig->div - 1U));
            }
            else /* Set other module clock source in clock mux0~2 */
            {
                DEVICE_ASSERT((HSEDIV2_CLK == moduleClkConfig->source)      \
...
void CKGEN_DRV_GetSystemClockSource(sys_clk_config_t *sysClkConfig)
{
    ...
    /* Core divider */
    sysClkConfig->dividers[0U] = (uint16_t)((CKGEN_GetCurrentSysClockDividerRatio(CKGEN)) + 1U);

    /* Bus divider */
    sysClkConfig->dividers[1U] = (uint16_t)((CKGEN_GetCurrentBusClockDividerRatio(CKGEN)) + 1U);
}
...
status_t CKGEN_DRV_UpdateConfiguration(uint8_t targetConfigIndex, clock_manager_policy_t policy)
{
    ...
    DEVICE_ASSERT(targetConfigIndex < g_clockState.clockConfigNum);       /* Clock configuration index is out of range. */
    ...
    for (callbackIdx = 0U; callbackIdx < g_clockState.callbackNum; callbackIdx++)
    {
        callbackConfig = g_clockState.callbackConfig[callbackIdx];
        if ((callbackConfig) && (callbackConfig->callbackType != CLOCK_MANAGER_CALLBACK_AFTER))
        {
    ...
    {
        /* clock mode switch. */
        ret = CKGEN_DRV_SetConfiguration(g_clockState.configTable[targetConfigIndex]);
    ...
    if (successfulSetConfig)
    {
        ...
        {
            callbackConfig = g_clockState.callbackConfig[callbackIdx];
            if ((callbackConfig) && (callbackConfig->callbackType != CLOCK_MANAGER_CALLBACK_BEFORE))
            {
                if (STATUS_SUCCESS != (*callbackConfig->callback)(&notifyStruct, callbackConfig->callbackData))
                {
                    g_clockState.errorCallbackIndex = callbackIdx;
